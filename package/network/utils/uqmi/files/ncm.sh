...
proto_ncm_setup() {
	local interface="$1"
	local mtu mac method device pdp modem pdptype sim dhcp dhcpv6 $PROTO_DEFAULT_OPTIONS IFACE4 IFACE6 delay
	...
	json_get_vars mtu mac method device pdp modem pdptype sim dhcp dhcpv6 delay ip4table ip6table $PROTO_DEFAULT_OPTIONS

	mdm_ubus_obj="$(find_mdm_ubus_obj "$modem")"
	[ -z "$mdm_ubus_obj" ] && echo "Correct gsm.modem object not found. Downing $interface interface" && ifdown $interface
