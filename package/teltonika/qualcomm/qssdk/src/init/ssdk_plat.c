...
static int miibus_get(a_uint32_t dev_id)
{
	...
	if (!mdio_node) {
		pr_debug("No MDIO node found in DTS!\n");
		return 0;
	}

	mdio_plat = of_find_device_by_node(mdio_node);
	if (!mdio_plat) {
		pr_debug("cannot find platform device from mdio node\n");
		return 1;
	}
	...
	if(ssdk_dt_global.ssdk_dt_switch_nodes[dev_id]->switch_reg_access_mode == HSL_REG_LOCAL_BUS)
	{
		...
		if (!mdio_data) {
					pr_debug("cannot get mdio_data reference from device data\n");
                	return 0;
        	}
	...
	*/
	...
	if (!miibus) {
		pr_debug("cannot get mii bus reference from device data\n");
		return 1;
	}
...
void ssdk_dts_port_scheduler_dump(a_uint32_t dev_id)
{
	...
	pr_debug("===============================port_scheduler_resource===========================\n");
	pr_debug("portid     ucastq     mcastq     10sp     10cdrr     10edrr     11cdrr     11edrr\n");
	for (i = 0; i < SSDK_MAX_PORT_NUM; i++)
	{
		...
		pr_debug("%6d%11s%11s%9s%11s%11s%11s%11s\n", i, srcmsg[0], srcmsg[1], srcmsg[2], srcmsg[3],
...
void ssdk_dts_l0scheduler_dump(a_uint32_t dev_id)
{
	...
	pr_debug("==========================l0scheduler_cfg===========================\n");
	pr_debug("queue     portid     cpri     cdrr_id     epri     edrr_id     sp_id\n");
	for (i = 0; i < SSDK_L0SCHEDULER_CFG_MAX; i++)
	{
		...
		if (scheduler_cfg->valid == 1)
			pr_debug("%5d%11d%9d%12d%9d%12d%10d\n", i, scheduler_cfg->port_id,
		...
		char *buf)
{
	...
	for (dev_id = 0; dev_id < ssdk_dt_global.num_devices; dev_id ++)
	{
		...
		pr_debug("        switch_access_mode = <%s>\n", dt_cfg->reg_access_mode);
		pr_debug("        switch_cpu_bmp = <0x%x>\n", dt_cfg->port_cfg.cpu_bmp);
		pr_debug("        switch_lan_bmp = <0x%x>\n", dt_cfg->port_cfg.lan_bmp);
		pr_debug("        switch_wan_bmp = <0x%x>\n", dt_cfg->port_cfg.wan_bmp);
		pr_debug("        switch_mac_mode = <0x%x>\n", dt_cfg->mac_mode);
		pr_debug("        switch_mac_mode1 = <0x%x>\n", dt_cfg->mac_mode1);
		pr_debug("        switch_mac_mode2 = <0x%x>\n", dt_cfg->mac_mode2);
		pr_debug("        bm_tick_mode = <0x%x>\n", dt_cfg->bm_tick_mode);
		pr_debug("        tm_tick_mode = <0x%x>\n", dt_cfg->tm_tick_mode);

		pr_debug("ess-psgmii\n");
		pr_debug("        reg = <0x%x 0x%x>\n", dt_cfg->psgmiireg_base_addr, dt_cfg->psgmiireg_size);
		pr_debug("        psgmii_access_mode = <%s>\n", dt_cfg->psgmii_reg_access_str);

		pr_debug("ess-uniphy\n");
		pr_debug("        reg = <0x%x 0x%x>\n", dt_cfg->uniphyreg_base_addr, dt_cfg->uniphyreg_size);
		pr_debug("        uniphy_access_mode = <%s>\n", dt_cfg->uniphy_access_mode);
...
int ssdk_sysfs_init (void)
{
	...
	if (!ssdk_sys) {
		pr_debug("Failed to register SSDK sysfs\n");
		return ret;
	}
	...
	if (ret) {
		pr_debug("Failed to register SSDK dev id SysFS file\n");
		goto CLEANUP_1;
	}
	...
	if (ret) {
		pr_debug("Failed to register SSDK log level SysFS file\n");
		goto CLEANUP_2;
	}
	...
	if (ret) {
		pr_debug("Failed to register SSDK switch counter SysFS file\n");
		goto CLEANUP_3;
	}
	...
	if (ret) {
		pr_debug("Failed to register SSDK switch counter bytes SysFS file\n");
		goto CLEANUP_4;
	}
	...
	if (ret) {
		pr_debug("Failed to register SSDK switch show dts SysFS file\n");
		goto CLEANUP_5;
	}
...
ssdk_plat_init(ssdk_init_cfg *cfg, a_uint32_t dev_id)
{
	...
	if(ssdk_dt_global.ssdk_dt_switch_nodes[dev_id]->uniphy_reg_access_mode == HSL_REG_LOCAL_BUS) {
		...
		if (!hppe_uniphy_addr) {
			pr_debug("%s ioremap fail.", __func__);
	...
	if(ssdk_dt_global.ssdk_dt_switch_nodes[dev_id]->psgmii_reg_access_mode == HSL_REG_LOCAL_BUS) {
		/* Mem region is already reserved by platform bus while scanning device tree
		 * and creating platform_device instances 
		 */
		/*if (!request_mem_region(ssdk_dt_global.ssdk_dt_switch_nodes[dev_id]->psgmiireg_base_addr,
			...
			pr_debug("%s Unable to request psgmii resource.", __func__);
			return -1;
		}*/
