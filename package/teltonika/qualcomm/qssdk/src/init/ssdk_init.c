...
qca_switch_init(a_uint32_t dev_id)
{
	...
	for (i = 0; i < AR8327_NUM_PORTS; i++) {
		...
		if (SSDK_CURRENT_CHIP_TYPE == CHIP_SHIVA) {
			...
			fal_qos_port_mode_set(dev_id, i, FAL_QOS_DSCP_MODE, 1);
			#endif
			#endif
...
static int
qca_ar8327_sw_delay_reset(struct switch_dev *dev,
                         ...
                         struct switch_val *val)
{
	...
	for (i = 0; i < AR8327_NUM_PHYS - 1; i++) {
		...
		/* start autoneg*/
		priv->phy_write(dev_id, i, MII_ADVERTISE, ADVERTISE_ALL |
						     ADVERTISE_PAUSE_CAP | ADVERTISE_PAUSE_ASYM);
		//phy reg 0x9, b10,1 = Prefer multi-port device (master)
		priv->phy_write(dev_id, i, MII_CTRL1000, (0x0400|ADVERTISE_1000FULL));

		priv->phy_write(dev_id, i, MII_BMCR, BMCR_RESET | BMCR_ANENABLE);

		priv->phy_dbg_read(dev_id, i, 0, &phy_val);
		phy_val &= (~(1<<12));
		priv->phy_dbg_write(dev_id, i, 0, phy_val);
