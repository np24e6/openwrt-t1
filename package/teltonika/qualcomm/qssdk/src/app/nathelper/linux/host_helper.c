...
char nat_lan_dev_list[IFNAMSIZ*4] = "br-lan eth0";
char nat_wan_dev_list[IFNAMSIZ*4] = "eth1";
    ...
    if (!(get_aclrulemask() & (1 << S17_ACL_LIST_DROUTE)))
    {
                ...
                (nf_athrs17_hnat_wan_type != NF_S17_WAN_TYPE_PPPOES0))
        {
            ...
            if (next_hop != 0)
            {
                ...
                if (rv != SW_OK)
                {
                    pr_debug("%s: IP_HOST_GET error... (non-existed host: %08x?) \n", __func__, next_hop);
                    /* add into the nh_ent */
        ...
        {
            pr_debug("multi_route_indev %p nf_athrs17_hnat_wan_type %d\n", multi_route_indev, nf_athrs17_hnat_wan_type);
        }
    }
    /* end next hop (s) */
...
static struct in6_addr* get_ipv6_default_gateway(void)
{
    ...
    struct rt6_info *rt = rt6_lookup(&init_net, &des_addr, NULL, 0, NULL, 0);
...
static int add_pppoev6_host_entry(void)
{
    ...
    if (0 != ppp_sid)
    {
        if ((ppp_sid == ppp_sid2)||(0 == ppp_sid2)) /* v4 and v6 have the same session id */
        {
            ...
            if (rv != SW_OK)
            {
                pr_debug("%s: IP_HOST_GET error (0x%08x)\n", __func__, ppp_peer_ip);
...
static sw_error_t setup_interface_entry(char *list_if, int is_wan)
{
    ...
    while ((dev_name = strsep(&list_all, " ")) != NULL)
    {
        ...
        if (NULL == nat_dev)
        {
            // pr_debug("%s: Cannot get device %s by name!\n", __FUNCTION__, dev_name);
            setup_error = SW_FAIL;
            continue;
        }
#if defined (CONFIG_BRIDGE)
	...
	while(1) {
		...
		if(tmp_vid != 0) {
			if(entry.mem_ports & nat_wan_port) {
				...
				nat_wan_vid = tmp_vid;
				snprintf(nat_wan_dev_list, IFNAMSIZ*4, "eth0.%d eth1 pppoe-wan", tmp_vid);
			} else {
				/*lan port*/
				HNAT_PRINTK("lan port vid:%d\n", tmp_vid);
				nat_lan_vid = tmp_vid;
				snprintf(nat_lan_dev_list, IFNAMSIZ*4, "eth0.%d eth0 br-lan",
					tmp_vid);
			}
		}
	}
...
static int setup_all_interface_entry(void)
{
    ...
    //if (0 == setup_lan_if)
    {
        ...
        if (SW_OK == setup_interface_entry(nat_lan_dev_list, 0))
        {
            ...
            //pr_debug("Setup LAN interface entry done!\n");
        }
    }
...
arp_in_bg_handle(struct nat_helper_bg_msg *msg)
{
    ...
    {
        pr_debug("Cannot get header info!!\n");
        return 0;
    }
#else
    if(dev_is_lan) {
    ...
    if(fal_fdb_entry_search(0, &entry) == SW_OK) {
        ...
        pr_debug("not find the FDB entry\n");
    }
#endif
    ...
    while (1)
    {
                ...
                (host_entry_p->counter_en))
        {
            if (0 != host_entry_p->packet)
            {
                ...
                pr_debug("Update WAN port hostentry!\n");
                IP_HOST_ADD(0, host_entry_p);
            }
            else
            {
                pr_debug("Del WAN port hostentry!\n");
                ...
                (NAPT_NEXT(0, FAL_NAT_ENTRY_PUBLIC_IP_EN ,&pub_napt) != 0))
        {
            ...
            pr_debug("ARP id 0x%x: Cannot find NAPT entry!\n", host_entry_p->entry_id);
            IP_HOST_DEL(0, FAL_IP_ENTRY_IPADDR_EN, host_entry_p);
            continue;
        }
        // arp entry is using, update it.
...
static unsigned int ipv6_bg_handle(struct nat_helper_bg_msg *msg)
{
    ...
    if(PROTO_ICMPV6 == iph6->nexthdr)
    {
        if(NEIGHBOUR_AD == icmp6->icmp6_type)
        {
            ...
            if ((0 == vid)||(0 == sport))
            {
                pr_debug("Error: Null sport or vid!!\n");
                return 0;
            }
#else
            if(dev_is_lan) {
...
void host_helper_init(void)
{
	...
	if (((nat_chip_ver & 0xffff)>>8) == NAT_CHIP_VER_8337 ||
		...
		pr_debug("host_helper_init start\n");
    ...
    for_each_net(n) {
    	rv = nf_register_net_hook(n, &arpinhook);
	printk(KERN_EMERG "nf_register_net_hook %d\n", rv);
    }
...
void host_helper_exit(void)
{
    ...
    napt_procfs_exit();
    for_each_net(n)
    	nf_unregister_net_hook(n, &arpinhook);
#if 0
#ifdef CONFIG_IPV6_HWACCEL
